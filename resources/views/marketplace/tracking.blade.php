            ...
            <div class="relative bg-white shadow-xl rounded-lg overflow-hidden">
                ...
                <div class="relative bg-white p-6">
                        ...
                        @if($order->payment_type === 'down_payment')
                            ...
                            <div class="flex justify-between items-center pb-3 border-gray-200 border-b border-dashed">
                                ...
                                <span class="font-medium text-emerald-600">Rp
                                    {{ number_format($order->amount_paid, 0, ',', '.') }}</span>
                            </div>
                    ...
                    @if($order->status === \App\Enums\OrderStatus::PendingFinalPayment && $order->payment_type === 'down_payment')
                        <div class="mt-6 pt-6 border-gray-200 border-t border-dashed">
                            <div class="flex items-center gap-3 mb-6">
                                <span
                                    class="flex justify-center items-center bg-gray-900 rounded-full w-6 h-6 font-bold text-white text-xs">!</span>
                                <h3 class="font-bold text-gray-500 text-xs md:text-sm uppercase tracking-widest">Pelunasan via
                                    QRIS</h3>
                            </div>

                            @if(session('success'))
                                <div class="bg-emerald-50 mb-6 p-4 border border-emerald-200 rounded-lg text-emerald-700 text-sm">
                                    {{ session('success') }}
                                </div>
                            @endif
                            @if(session('error'))
                                <div class="flex items-start gap-3 bg-red-50 mb-6 p-4 border-red-500 border-l-4 rounded-r-lg">
                                    <svg class="mt-0.5 w-5 h-5 text-red-500 shrink-0" xmlns="http://www.w3.org/2000/svg"
                            ...
                            {{-- QRIS block --}}
                            <div class="flex flex-col gap-6 mb-6 py-6 border-gray-200 border-y border-dashed">
                                <div
                                    class="flex justify-center bg-gray-50 py-4 border border-gray-200 rounded-lg w-full h-auto shrink-0">
                                    <img src="{{ asset('assets/qris.webp') }}" alt="QRIS Himti"
                                        class="rounded-2xl w-48 object-center object-contain">
                                </div>
                                <div class="flex-1 w-full text-center">
                                    ...
                                    <div
                                        ...
                                        bukti pelunasan di bawah ini.
                                    </div>
                                </div>
                            </div>

                            <form action="{{ route('marketplace.order.final-payment', $order->tracking_token) }}" method="POST"
                                enctype="multipart/form-data" id="payment-form">
                                @csrf
                                {{-- Upload Bukti Pembayaran --}}
                                <div>
                                    <label class="block mb-2 font-semibold text-gray-700 text-sm">Upload Bukti Pelunasan <span
                                    ...
                                    <div class="group relative bg-gray-50 p-6 border-2 border-gray-300 hover:border-blue-500 border-dashed rounded-xl text-center transition-colors cursor-pointer"
                                        ...
                                        <div class="text-gray-600 text-sm"><strong class="text-blue-700">Pilih file</strong>
                                            atau drag &amp; drop di sini</div>
                                        <div class="mt-2 text-gray-500 text-xs">PNG, JPG, JPEG — Maks. 5 MB</div>
                                        <input id="final_payment_proof" name="final_payment_proof" type="file"
                                    ...
                                    <div class="flex justify-between items-center bg-green-50 mt-4 p-4 border border-green-200 rounded-xl"
                                        ...
                                        <div class="flex items-center gap-3 overflow-hidden">
                                            <svg class="w-6 h-6 text-green-600 shrink-0" viewBox="0 0 20 20"
                                                fill="currentColor">
                                                <path fill-rule="evenodd"
                                                    d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z"
                                                    clip-rule="evenodd" />
                                            </svg>
                                            <span class="font-medium text-gray-900 text-sm truncate" id="file-name"></span>
                                        </div>
                                        <button type="button"
                                ...
                                <button type="submit" id="submit-btn"
                                    class="flex justify-center items-center gap-2 bg-blue-700 hover:bg-blue-600 disabled:opacity-50 mt-6 px-8 py-3.5 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 w-full font-semibold text-white text-sm transition-colors disabled:cursor-not-allowed">
                                    <svg id="submit-spinner" class="hidden w-5 h-5 animate-spin"
